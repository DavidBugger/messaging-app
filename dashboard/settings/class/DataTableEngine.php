<?php
class DataTableEngine extends Model
{
	public $id    = "";
	public $draw   = "";
	public $start  = "";
	...
	public function global_generic_report_table($data, $filter_with_table, $columner, $pk, $filter = "")
	{ //filter is an EXTRA FILTER
		...
		$result   = $this->global_db_connect($sql);
	...
	public function generic_report_table($data, $filter_with_table, $columner, $pk, $filter = "", $groupby_db = "")
	{ //filter is an EXTRA FILTER
		// $table_name    = $table_name;
		$this->draw    = isset($data['draw']) ?   $data['draw']  : "";
		$this->start   =  isset($data['start']) ?  $data['start']  : "";
	...
	public function generic_select_report_table($data, $select, $filter_with_table, $columner, $pk, $filter = "", $groupby_db = "", $order_by_table_alias = "", $order_by_column = "", $order_by_column_1 = "")
	{ //filter is an EXTRA FILTER
		// $table_name    = $table_name;
		$this->draw    = isset($data['draw']) ?   $data['draw']  : "";
		$this->start   =  isset($data['start']) ?  $data['start']  : "";
		...
		if ($order_by_column != '') {
			$order_by_column = $order_by_table_alias . $order_by_column;
		} else {
			$order_by_column = $order_by_table_alias . $order_db;
		}

		if ($order_by_column_1 != '') {
			$order_by_column_1 = ',' . $order_by_column .' '. $this->dirs;
		} else {
			$order_by_column_1 = '';
		}
		...
		$sql = "SELECT $select FROM $filter_with_table  AND " . $this->prepareSearch($columner, $this->search) . " " . $groupby_db . " order by " . $order_by_column . " " . $this->dirs . " $order_by_column_1 LIMIT " . $this->start . ", " . $this->length;

		$result   = $this->db_connect($sql);

		$sql_without_limit = "SELECT $pk FROM $filter_with_table AND " . $this->prepareSearch($columner, $this->search) . " " . $groupby_db . " order by " . $order_by_column . " " . $this->dirs . $order_by_column_1;
	...
	public function generateTable($data, $table_name, $columner, $pk, $filter = "")
	{
		$table_name    = $table_name;
		$this->draw    = $data['draw'];
	...
	public function generic_multi_table($data, $table_name, $columner, $pk, $join_arr, $join_type = "JOIN", $filter = "")
	{
		$table_name    = $table_name;
		$this->draw    = $data['draw'];
		$this->start   = $data['start'];
	...
	{
		$pagination = ($connection == "") ? $this->db_connect($sql_without_limit, false)  : $this->global_db_connect($sql_without_limit, false);
	...
	public function db_connect($sql, $object = true)
	{
		...
		if ($object) {
			...
			file_put_contents('logger/db_connect.php', $sql . PHP_EOL, FILE_APPEND | LOCK_EX);
			return $count;
		}
	}

	public function global_db_connect($sql, $object = true)
	{
		// if you are performig a UPDATE query; you will need to set $object == false
		$result = $this->conn->query($sql);
		$result->execute();
